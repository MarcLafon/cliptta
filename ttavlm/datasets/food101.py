...
class Food101Dataset(Dataset):
    def __init__(
        ...
        db = lib.load_json(os.path.join(dirname(abspath(__file__)), "files", "data_suite_splits", "split_zhou_Food101.json"))
        db = db[split]
        ...
        self.names = []
        for x in db:
            self.paths.append(os.path.join(self.root, "images", x[0]))
