...
class StanfordCarsDataset(Dataset):
    def __init__(
        ...
        db = lib.load_json(os.path.join(dirname(abspath(__file__)), "files", "data_suite_splits", "split_zhou_StanfordCars.json"))
        db = db[split]
        ...
        self.names = []
        for x in db:
            self.paths.append(os.path.join(self.root, x[0]))
...
if __name__ == "__main__":
    dts = StanfordCarsDataset(root="/local/DEEPLEARNING/dataset_suite", split="train")
    print(dts[0])
