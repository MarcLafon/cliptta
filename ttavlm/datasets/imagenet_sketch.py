...
class ImagenetSketchDataset(ImageFolder):
    folder_dir = "sketch"
    download_url = "https://drive.google.com/u/0/uc?id=1Mj0i5HBthqH1p_yeXzsg22gZduvgoNeA&export=download"

    def __init__(self, root: str, *args: ArgsType, download: bool = False, **kwargs: KwargsType) -> None:
        self.root_folder = root
        self.root = os.path.join(self.root_folder, self.folder_dir)
        if download:
            self.download()
        super().__init__(self.root, *args, **kwargs)
        self.shift_type = "imagenet_sketch"
